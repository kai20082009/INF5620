"""
	...
	The problem is given as 
		u'' + (w^2)u = f(t).
	The discretized notation is written as 
		[D_tD_t u + (w**2)u = f]^f, which is equivalent to 
	...
	The derivative of u(0) can be used to define the expression for u(-1). That is 
		D_2t u'(0) = [u(1) - u(-1)]/2dt = V, and this implies that u(-1) = u(1) - 2dt*V-
	...
	u(1) = 1/2 * ( 2 + (w*dt)**2 )I + f*dt**2) + dt*V
        ...
        Problem b)
	We have now that u_e = c*t + d. With initial conditions, we have that:
		
		u_e(0) = I = d
		u_e'(0) = V = c
	Therefore we have that:
		u_e(t) = Vt + I, and we inserting  this in the ODE, we 'll get
		f(t) = w**2(VT + I)
	
		[D_tD_tt]^n = (t^n+1 - 2t^n + t_^n-1)/dt^2. Since we know that every t step equals to each other,
				that means t^n+1 = t^n-1 = t^n.
				Moreover, t^n+1 + t^n-1 - 2t^n = 0.
	Thus, [D_tD_t(t)]^n = 0.
		[D_tD_t(ct+d)]^n = c[D_tD_t(t)]^n + [D_tD_t(d)]^n = 0 + 0 = 0. 
